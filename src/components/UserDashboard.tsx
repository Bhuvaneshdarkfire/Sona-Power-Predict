...
import {
  ...
  XCircle, Clock, FileArchive, Loader2, Lock
} from 'lucide-react';
...
import { uploadSubmission, getSubmissions, getSubmissionStatus, getTeamPredictions, getUploadSchedule } from '../services/api';
...
const UserDashboard = () => {
  ...
  const loadUploadSchedule = async () => {
    try {
      const data = await getUploadSchedule();
      setUploadOpen(data.isOpen);
      setUploadMessage(data.message);
    } catch {
      // If schedule fails to load, default to allowing uploads
      setUploadOpen(true);
    }
  };
  ...
  const handleUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Frontend gate: check if uploads are open
    if (uploadOpen === false) {
      setModal({ isOpen: true, title: '🔒 Uploads Closed', message: uploadMessage || 'Uploads are not open right now. Check the timeline for the next upload window.', type: 'error' });
            ...
            <div className="bg-white p-7 rounded-2xl border border-gray-200 shadow-sm">
              ...
              <div className="flex items-center gap-4">
                <input
                  ...
                  className="hidden"
                  id="model-upload"
                  disabled={uploadOpen === false || uploading}
                />
                <label
