...
const { isUploadWindowOpen } = require('../config/uploadSchedule');
 ...
 * Enforces upload window schedule — blocks uploads outside allowed dates/times.
 */
async function uploadSubmission(req, res) {
    // ── Check upload window ──
    const windowStatus = isUploadWindowOpen();
    if (!windowStatus.open) {
        // Cleanup uploaded file if multer already saved it
        if (req.file?.path) {
            try { fs.unlinkSync(req.file.path); } catch { /* ignore */ }
        }
        return res.status(403).json({
            error: 'Uploads are not open right now.',
